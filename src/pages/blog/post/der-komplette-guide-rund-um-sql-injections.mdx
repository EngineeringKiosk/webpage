...
* Datendiebstahl: Sensible Daten wie Anmeldedaten, Kreditkartennummern und persönliche Informationen gelangen in die falschen Hände
* Änderung von Daten: Datenverlust oder Datenbeschädigung
* Kompromittierung von Anwendungen: Die Kontrolle über eine Webanwendung kann übernommen werden sowie Malware oder Ransomware eingeschleust werden
* Schädigung des Rufs: Negative Publicity und Verlust von Kundenvertrauen und Geschäftsmöglichkeiten.
* Verstöße gegen gesetzliche Vorschriften: HIPAA, PCI DSS oder GDPR.

Technisch kann ein Angriff wie folgt durchgeführt werden:
...
1. Das Statement `SELECT user, password FROM tbl_user WHERE user = ?` wird an den Datenbank-Server gesendet, der die Abfrage für die aktuelle Session speichert und eine ID zurückliefert.
2. Dem Parameter `?` (als Wert für das Feld `user`) die Zeichenkette aus der Variable `username` zugewiesen.
3. Der Client sendet die Zeichenkette und die ID der Abfrage an DB Server, der die eigentliche Abfrage erstellt und ausführt.
...
<PodcastEpisodeBlogpostAd episode="58" />

### Limitierte Privilegien für Datenbank-Benutzer (Prinzip der geringsten Berechtigung)

Welche Privilegien hat der verwendete Datenbank Benutzer? Kann dieser neue Benutzer anlegen? Ist dies eine notwendige Operation der Anwendung?

Eine Kontrolle der benötigten Datenbank-Privilegien des Datenbank-Benutzers ist stets sinnvoll. Es sollte das Prinzip der geringsten Berechtigung befolgt werden, indem der Benutzerzugriff auf die Ressourcen beschränkt wird, die für die Ausführung der Aufgaben benötigt werden. Dies macht möglichen Angreifern das Leben schwerer.
